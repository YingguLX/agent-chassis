...
- Full automatic quality chain: full automatic quality-control mode serially converges through `public impact domain`, `global security domain`, `structural coverage domain`, and `manual output domain` to the current ordinary task's acceptance goal; it must not be rewritten as a user-operable command or manual phase-selection entry.
