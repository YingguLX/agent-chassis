...
Markdown emphasis levels must be used according to constraint strength: *italic* is used for key terms, key concepts, and easily misread boundaries; **bold** is used for strong constraints and must/must not/can only execution requirements; ***bold italic*** is used for red lines, hard gates, stop conditions, and unavoidable items. Markdown special syntax outside plain text, including emphasis, backticks, quotes, tables, lists, fenced code blocks, and links, **must use spaces correctly**. When maintaining `AGENTS.md` or similar project-level agent constraint rules, strong constraints, key constraints, high-risk boundaries, stop-confirmation conditions, and prohibitions must be checked for correct `*...*`, `**...**`, and `***...***` grading. User-facing replies should also use the three emphasis levels according to importance: ordinary explanations use little or no emphasis, important conclusions use `**...**`, and high-risk, stop-confirmation, or non-violable items use `***...***`. Do not mechanically emphasize every ordinary sentence, do not let over-emphasis make "everything emphasized" equivalent to "nothing emphasized", and do not let true hard gates, stop conditions, or unavoidable items degrade into ordinary un-emphasized text.
...
- `agents/RULES.md` is the detailed explanation authority, carrying the document maintenance framework, fixed protocol constant semantics, automatic quality guard, unobtrusive quality guard, task risk level, automatic quality-loop level, subagent lifecycle, documentation release rules, and quality check details.
