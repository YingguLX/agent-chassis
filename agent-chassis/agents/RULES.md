...
- ***Preserve Text Format***: Before modifying code or documentation, the target text file's encoding, line-ending format, EOF newline, and trailing-whitespace baseline must be read. Writes must preserve the same file's original encoding and line-ending format, and post-write rechecks must be performed. Mixing CRLF, LF, or other line-ending formats in the same file is prohibited.
...
- Public entries, APIs, commands, endpoints, configurations, data formats, user interface entries, or usage contracts defined in `{{%MAIN_PUBLIC_ENTRIES}}` must not be renamed, deleted, or reordered casually.
...
- C/C++ C linkage wrapper matrixization must be organized in this order: `template static inline __xmacro_helper_accel_$#action$_t` function template -> `$#LIBNAME$_API_MATRIX(X)` X-Macro matrix -> `$#LIBNAME$_DEFINE_$#ACTION$` action macro -> execute `$#LIBNAME$_API_MATRIX($#LIBNAME$_DEFINE_$#ACTION$);` per action inside `extern "C"` -> `#undef $#LIBNAME$_API_MATRIX` and all `#undef $#LIBNAME$_DEFINE_$#ACTIONn$`. `$#LIBNAME$`, `$#ACTION$`, `$#ACTION0$`, `$#ACTION1$`, `$#ACTION2$`, and `$#action$` are only code-skeleton metvariables in this document for explaining naming-fragment substitution; they are not AgentChassis template placeholders, are not added to the template placeholder list, are not added to fixed protocol fields, and must not enter real C/C++ source code as literals.
...
| `AGENTS.md` | Only carries the agent control entry, automatic quality guard, unobtrusive quality guard, load order, version rules, and minimal context. |
| `agents/RULES.md` | Only carries engineering rules, coding-style constraints, document maintenance rules, automatic quality guard, unobtrusive quality guard, automatic quality loop rules, and formal user manual writing rules jointly followed by developers and agents. |
...
When preparing a task plan, the session agent must explicitly record the following fields: `task risk level`, judgment evidence, `automatic quality-loop level`, inclusion reason, task complexity/scale assessment, blocking issue check result, confirmation status, Git/worktree baseline, safety and sensitive-information boundary, matters that require user confirmation, expected chain levels, subagent orchestration, capability detection time point, dynamically detected actual concurrency cap, code-index capability status, code-index result usage boundary, code-index failure fallback method, task dependency graph, explicit dependencies, implicit dependencies, cycle dependency check result, document read/write ownership, file responsibility boundary, per-file responsibility matrix, parallel delegation strategy, main-agent context retention scope, hard total cap, detection-failure stop action, rollback method, verification method, recovery entry, stop conditions, and acceptance output. Plan text, historical requests, template body text, or automatic quality guard must not constitute authorization across tasks, rounds, scopes, or high-risk operations. Only current explicit user authorization, protected-gate confirmation, or narrow exceptions defined by rules can be used as the basis for write, Git, external-resource, sensitive-file, dependency, database, deployment, or release actions. When risk judgment cannot be made reliably, full automatic quality guard mode must be selected. If full automatic quality guard mode is still insufficient to explain conflict, permission, or responsibility boundaries, execution must stop and request user adjudication.

Before implementing an ordinary task plan, the plan-review gate must be executed item by item: whether the requirement background is known, whether the final goal is clear, whether inputs and outputs have clear boundaries, whether there is any conflict with real conditions, whether any unresolved implicit assumption remains, whether any detail is still missing or improper, whether any remaining blocker affects plan execution, and whether any noise outside this requirement remains. If any item has not converged, the issues must be listed immediately and confirmation must be requested from the user; unrelated noise must be removed from the plan, implementation, acceptance, and report scope and must not be used as an execution basis. When large refactoring, adding/upgrading/removing dependencies, changes to interface signatures or public contracts not otherwise involved, module-structure migration, bulk renaming, or other high-impact action beyond the current task boundary is involved, it must be listed under `matters that require user confirmation`; without explicit user confirmation, it must not be treated as a default approach, implicit premise, or implementation step.
...
#### 10.2.1 Underlying Fact Derivation Protocol

Underlying fact derivation is a general planning discipline embedded in the ordinary task lifecycle. It is not a user entry, not a manual quality-stage selector, must not restore a user-manual-trigger quality command model, and must not let the eight automatic quality domains become quality tasks users can invoke separately. It only constrains the derivation process from user goal to executable plan, must be embedded in the ordinary task's planning, implementation, verification, and report, and must preserve automatic/unobtrusive quality guard semantics.
...
| --- | --- | --- |
| `incremental difference domain` | Incremental semantic risks, interface-change risks, test gaps, and document alignment gaps caused by the currently tracked staged/unstaged diff | Does not perform system CWE global security domain, whole-repository static analysis domain, full public impact domain, Markdown proofreading, or formal user manual generation |
...
| `public impact domain` | Public entries, API usage contracts, ABI binary boundaries, caller chains, version impact inference, and coverage of the incremental semantic, interface, test-gap, and document-alignment risk slice caused by the currently tracked diff | Does not perform general code-defect static analysis domain, security threat modeling, Markdown proofreading, or documentation release domain |
...
The reference table of contents should naturally evolve from the following hierarchy. Real headings and anchors must be generated from real headings in the formal manual and adjusted with the public exposure surface and applicable public entry matrix of `{{%MAIN_PUBLIC_ENTRIES}}`. The example preserves the document H1 and `## Table of Contents` as manual structure, and table-of-contents entries themselves start by default from real body headings at `##` and below:
...
1. APIs, CLIs, SDKs, services, plugins, protocols, configurations, user interfaces, data formats, model entries, deployment entries, operations entries, and documentation entries publicly declared in `{{%MAIN_PUBLIC_ENTRIES}}`.
...
The above order is a recommended organization principle, not a fixed table-of-contents list. The number, names, and order of concrete module chapters should evolve with the public facts and applicable public entry matrix of `{{%MAIN_PUBLIC_ENTRIES}}`.
